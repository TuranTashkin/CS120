...
class LetterStatus {
    constructor() {
        this.status = {"A": 0, "B": 0, "C": 0, "D": 0, "E": 0, "F": 0, "G": 0, "H": 0, "I": 0, "J": 0, 
    ...
    get(letter) {
        return this.status[letter];
    }
    
    // set status of a letter
    set(letter, value) {
        this.status[letter] = value;
    }
}

let letterStatus = new LetterStatus();
...
// handle the end game
const endGame = () => {
    document.getElementById('endGames').classList.remove('hidden');
}
...
function colorWords() {
    ...
    wordSplit.forEach((letter, i) => {
        if(letter == guessSplit[i]){
            ...
            letterStatus.set(guessSplit[i], 2);
            wordSplit[i] = null;
        }
    });    
    ...
    for(let i = 0; i < 5; i++) {
        if(guessSplit[i] !== null) {
            ...
            // iterate through the guess and current words to find any remaining duplicates and color yellow if there are any.
            for(let j = 0; j < 5; j++) {
                if(wordSplit[j] !== null && guessSplit[i] == wordSplit[j]) {

                    document.getElementById(`letter${currentRow}${i + 1}`).style.backgroundColor = 'yellow';
                    if(letterStatus.get(guessSplit[i]) !== 2) {
                        letterStatus.set(guessSplit[i], 1);
            ...
            if(!found && letterStatus.get(guessSplit[i]) < 1) {
                letterStatus.set(guessSplit[i], -1);
